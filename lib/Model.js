  ...
  if (options.condition) {
    ...
    if(options.conditionValues) {
      ...
      Object.keys(options.conditionValues).forEach(function (k) {
        ...
        if(attr) {
          req.ExpressionAttributeValues[':' + k] = val;
        } else {
...
Model.prototype.put = function(options, next) {
  ...
  function putItem() {
    model$.base.documentClient().put(item, function(err) {
      if(err) {
        deferred.reject(err);
      }
  ...
  try {
    ...
    for (var attrName in schema.attributes) {
      ...
      if (schema.attributes[attrName] && typeof schema.attributes[attrName].default === 'function') {
        defaultValue = schema.attributes[attrName].default(this);
      }
      itemRequest[attrName] = (this[attrName] !== null && this[attrName] !== undefined) ? this[attrName] : defaultValue;
    }
    ...
    schema.parseDynamo(this, schema.toDynamo(item.Item));
...
Model.get = function(NewModel, key, options, next) {
  ...
  getReq.Key[hashKeyName] = key[hashKeyName];

  if(schema.rangeKey) {
    var rangeKeyName = schema.rangeKey.name;
    schema.hashKey.toDynamo(key[rangeKeyName]);
    getReq.Key[rangeKeyName] = key[rangeKeyName];
  }
  ...
  function get () {
    ...
    newModel$.base.documentClient().get(getReq, function(err, data) {
      if(err) {
      ...
      schema.parseDynamo(model, schema.toDynamo(data.Item));
...
Model.update = function(NewModel, key, update, options, next) {
  ...
  updateReq.Key[hashKeyName] = key[hashKeyName];

  if(schema.rangeKey) {
    var rangeKeyName = schema.rangeKey.name;
    updateReq.Key[rangeKeyName] = key[rangeKeyName];
  }
    ...
    for (var putItem in updatePUT) {
      ...
      if (putAttr) {
        ...
        if (removeParams) {
          ...
          try {
            putAttr.toDynamo(val);
            operations.addSet(putItem, val);
          } catch (err) {
            deferred.reject(err);
            return deferred.promise.nodeify(next);
          }
      ...
      if(deleteAttr) {
        ...
        if(delVal !== null && delVal !== undefined) {
          try {
            ...
            operations.addRemove(deleteItem, delVal);
          } catch (err) {
            deferred.reject(err);
            return deferred.promise.nodeify(next);
          }
    ...
    for(var addItem in update.$ADD) {
      ...
      if(addAttr) {
        try {
          ...
          operations.addAdd(addItem, update.$ADD[addItem]);
        } catch (err) {
          deferred.reject(err);
          return deferred.promise.nodeify(next);
        }
  ...
  if (options.updateTimestamps && schema.timestamps) {
    ...
    operations.addIfNotExistsSet(createdAtLabel, createdAtDefaultValue);
    updatedAtAttribute.toDynamo(updatedAtDefaultValue);
    operations.addSet(updatedAtLabel, updatedAtDefaultValue);
  }
  ...
  if (options.createRequired) {
    for (var attributeName in schema.attributes) {
      ...
      if (attribute.required && // if the attribute is required...
        ...
        operations.addIfNotExistsSet(attributeName, defaultValue);
      }
    }
  }
  ...
  function updateItem () {
    ...
    newModel$.base.documentClient().update(updateReq, function(err, data) {
      if(err) {
        debug('Error returned by updateItem', err);
        return deferred.reject(err);
      }
      ...
      var model = new NewModel();
      model.$__.isNew = false;
      schema.parseDynamo(model, schema.toDynamo(data.Attributes));
...
Model.prototype.delete = function(options, next) {
  ...
  try {
    schema.hashKey.toDynamo(this[hashKeyName]);
    getDelete.Key[hashKeyName] = this[hashKeyName];

    if(schema.rangeKey) {
      var rangeKeyName = schema.rangeKey.name;
      schema.hashKey.toDynamo(this[rangeKeyName]);
      getDelete.Key[rangeKeyName] = this[rangeKeyName];
    }
  } catch (err) {
    deferred.reject(err);
    return deferred.promise.nodeify(next);
  ...
  function deleteItem() {
    ...
    model$.base.documentClient().delete(getDelete, function(err, data) {
      if(err) {
...
Model.batchGet = function(NewModel, keys, options, next) {
  ...
  getReq.Keys = keys.map(function (key) {
    var ret = {};
    ret[hashKeyName] = key[hashKeyName];

    if(schema.rangeKey) {
      var rangeKeyName = schema.rangeKey.name;
      ret[rangeKeyName] = key[rangeKeyName];
    }
    return ret;
  });
  ...
  function batchGet () {
    ...
    newModel$.base.documentClient().batchGet(batchReq, function(err, data) {
      if(err) {
      ...
      function toModel (item) {
        var model = new NewModel();
        model.$__.isNew = false;
        schema.parseDynamo(model, schema.toDynamo(item));
...
function batchWriteItems (NewModel, batchRequests) {
  ...
  var batchList = batchRequests.map(function (batchReq) {
    ...
    newModel$.base.documentClient().batchWrite(batchReq, function(err, data) {
      if(err) {
...
Model.batchDelete = function(NewModel, keys, options, next) {
  ...
  var batchRequests = toBatchChunks(newModel$.name, keys, MAX_BATCH_WRITE_SIZE, function(key) {
    var key_element = {};
    key_element[hashKeyName] = key[hashKeyName];

    if(schema.rangeKey) {
      key_element[schema.rangeKey.name] = key[schema.rangeKey.name];
    }

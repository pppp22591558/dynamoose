...
Model.prototype.put = function(options, next) {
  ...
  function putItem() {
    model$.base.documentClient().put(item, function(err, k) {
      if(err) {
        deferred.reject(err);
      }
  ...
  try {
    ...
    var item = {
      ...
      Item: this.$__.base.AWS.DynamoDB.Converter.unmarshall(schema.toDynamo(this)),
    };
...
Model.batchPut = function(NewModel, items, options, next) {
  ...
  var batchRequests = toBatchChunks(newModel$.name, items, MAX_BATCH_WRITE_SIZE, function(item) {
    return {
      PutRequest: {
        Item: newModel$.base.AWS.DynamoDB.Converter.unmarshall(newModel$.schema.toDynamo(item))
      }
    };
  });

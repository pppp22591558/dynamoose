  ...
  for(var name in dynamoObj) {
    ...
    if(attr) {
      ...
      if (this.attributeFromDynamo) {
        attrVal = this.attributeFromDynamo(name, attr.toDynamo(dynamoObj[name]), attr.parseDynamo.bind(attr));
      } else if (dynamoObj[name] && dynamoObj[name].wrapperName === 'Set') {
        attrVal = dynamoObj[name].values;
      } else if (dynamoObj[name] !== undefined && dynamoObj[name] !== null) {

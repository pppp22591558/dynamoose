...
Dynamoose.prototype.dax = function (daxClient) {
...
Dynamoose.prototype.documentClient = function() {
  ...
  if (this.daxClient) {
    debug('Setting dynamodb document client with dax');
    this.dynamoDocumentClient = new this.AWS.DynamoDB.DocumentClient({ service: this.daxClient });
  } else {
    debug('Getting dynamodb document client without DAX');
  }
  return this.dynamoDocumentClient;
};

...
Query.prototype.exec = function (next) {
  ...
  queryReq.KeyConditions[this.query.hashKey.name] = {
    AttributeValueList: [this.query.hashKey.value],
  ...
  if(this.query.rangeKey) {
    ...
    if(!rangeKey || rangeKey.values === undefined) {
      ...
      for (i = 0; i < rangeKey.values.length; i++) {
        ...
        keyConditions.AttributeValueList.push(val);
      }
    }
  }
    ...
    for(var name in this.filters) {
      ...
      if(filter.values) {
        for (i = 0; i < filter.values.length; i++) {
          val = filter.values[i];
          queryReq.QueryFilter[name].AttributeValueList.push(val);
        }
      }
    }
  }
  ...
  function query () {
    ...
    Model$.base.documentClient().query(queryReq, function(err, data) {
      if(err) {
